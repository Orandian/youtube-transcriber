...
export async function GET(request: NextRequest) {
  ...
  return Response.json({
    ...
    playabilitySnippet: html.match(/"playabilityStatus":\{"status":"([^"]+)"/)?.[1] ?? null,
    captionSnippet: html.includes('"captionTracks"') ? html.slice(html.indexOf('"captionTracks"'), html.indexOf('"captionTracks"') + 100) : null,
